  ...
  class Server
    ...
    def initialize
      ...
      @dispatcher = T.let(
        Thread.new do
          loop do
            ...
            # TODO: Better handle when we can't select an IO
            _, idle_io = IO.select(nil, @workers.map(&:input_write).shuffle)
            idle_io&.first&.write("#{Marshal.dump([job, @store])}\r\n\r\n")
          end
        end,
        Thread,
      )
      ...
      @responder = T.let(
        Thread.new do
          loop do
            idle_io, _ = IO.select(@workers.map(&:output_read).shuffle, nil, nil, 0.1)
            next unless idle_io

            output = T.must(idle_io.first).gets("\r\n\r\n", chomp: true)
            next if output.nil?

            result, request = T.cast(Marshal.load(output), [Result, T::Hash[Symbol, T.untyped]])
